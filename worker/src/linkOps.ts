 ...
 *
 * WHAT "CANNOT SETTLE" DOES AND DOES NOT MEAN
 * Stated precisely, because the short version reads stronger than what the
 * contract enforces:
 *
 *   • It holds for orders placed by REAL customers. A compromised backend
 *     cannot advance or cancel one, because it does not have that customer's
 *     browser key.
 *   • It does NOT stop a compromised backend placing an order with `customer`
 *     set to a key it controls, and marking that order paid itself. No USDC
 *     moves — the LP settles against their own bank — but it burns LP escrow and
 *     the merchant's daily allowance, and the strike system that bounds it lives
 *     in the same compromised backend.
 *   • The customer's key is generated by JavaScript served from the pay page. A
 *     compromise of that ORIGIN, not just of this worker, defeats the split.
 *
 * Both are acceptable — neither moves money — but they are the boundary of the
 * claim, and the claim should not be repeated without them.
 *
